...
public class BinderFunctionInfo {
     ...
     */
    private final Pair<my_lab.usage.BinderFunctionInfo.LinkerInfo, my_lab.usage.BinderFunctionInfo.LinkerInfo> linkInfo;
    ...
    BinderFunctionInfo(String name, Configuration pConfig, LogManager pLogger) {
        try {
            ...
            if (line != null) {
                ...
                my_lab.usage.BinderFunctionInfo.LinkerInfo[] lInfo = new my_lab.usage.BinderFunctionInfo.LinkerInfo[2];
                for (int i = 0; i < 2; i++) {
                    pOption = Splitter.on(":").splitToList(options.get(i));
                    int dereference = getNumOrDefault(pOption);
                    lInfo[i] = new my_lab.usage.BinderFunctionInfo.LinkerInfo(Integer.parseInt(pOption.get(0)), dereference);
                }
    ...
    private AbstractIdentifier constructIdentifier(
            final my_lab.usage.BinderFunctionInfo.LinkerInfo info,
            final CExpression left,
            final List<CExpression> params,
            IdentifierCreator creator) {
