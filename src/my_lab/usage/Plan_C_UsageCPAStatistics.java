...
public class Plan_C_UsageCPAStatistics implements Statistics {
    ...
    private org.sosy_lab.cpachecker.cpa.usage.UsageCPAStatistics.OutputFileType outputFileType = org.sosy_lab.cpachecker.cpa.usage.UsageCPAStatistics.OutputFileType.KLEVER;
    ...
    private Plan_C_ErrorTracePrinter errPrinter;
    ...
    public Plan_C_UsageCPAStatistics(
    ...
    public void printStatistics(
        ...
        Plan_C_UsageReachedSet uReached = (Plan_C_UsageReachedSet) reachedSet;
        ...
        if (printUnsafesInCaseOfUnknown || result != Result.UNKNOWN) {
            ...
            try {
                switch (outputFileType) {
                    case KLEVER3:
                        errPrinter = new Plan_C_Klever3ErrorTracePrinter(config, computer, cfa, logger, lockTransfer);
                        break;
                    case KLEVER:
                        errPrinter = new Plan_C_KleverErrorTracePrinter(config, computer, cfa, logger, lockTransfer);
                        break;
                    case KLEVER_OLD:
                        errPrinter =
                                new Plan_C_KleverErrorTracePrinterOld(config, computer, cfa, logger, lockTransfer);
                        break;
                    case ETV:
                        errPrinter = new Plan_C_ETVErrorTracePrinter(config, computer, cfa, logger, lockTransfer);
                        break;
                    default:
