...
public final class UsagePoint implements Comparable<UsagePoint> {
  ...
  private final List<CompatibleNode> compatibleNodes;   // 符合状态中可以比较的状态，例如ThreadState和LockState
  ...
  public boolean covers(UsagePoint o) {
    // access 'write' is higher than 'read', but only for nonempty locksets  锁集合为空时直接通过比较访问方式来决定覆盖关系
    if (access.compareTo(o.access) > 0) {
      return false;     // 当前的usagePoint的access比传入的UsagePoint的access更大
    }
    // 否则通过比较threadState和LockState的覆盖关系来得到整体的覆盖关系
    for (int i = 0; i < compatibleNodes.size(); i++) {
