...
public class ExpressionHandler extends DefaultCExpressionVisitor<Void, NoException> {
  ...
  private boolean isRelevantForAnalysis(AbstractIdentifier pId) {
    if (varSkipper.shouldBeSkipped(pId, fName)) {     // 如果不属于提前标记的需要跳过的，则是分析相关的
      return false;
    }

    if (pId instanceof LocalVariableIdentifier && pId.getDereference() <= 0) {      // 局部变量
      // we don't save in statistics ordinary local variables
      return false;
    }
    if (pId instanceof StructureIdentifier && !pId.isGlobal() && !pId.isDereferenced()) {     // 非全局的结构变量
      // skips such cases, as 'a.b'
      return false;
    }

    if (pId instanceof FunctionIdentifier) {      // 函数名Id
      return false;
    }
