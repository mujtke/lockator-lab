...
public class PredicateApplyOperator implements ApplyOperator {
  ...
  /**
   * 判断predicate状态的相容性
   * 重写compatible函数，让其能够接受AbstractState参数
   * 这里获取的是state1和state2的状态公式，因为state2不是投影
   */
  public boolean compatible(AbstractState pState1, AbstractState pState2) {
    ...
    BooleanFormula state1Formula = state1.getAbstractionFormula().asFormula();
    BooleanFormula state2Formula = state2.getAbstractionFormula().asFormula();
    BooleanFormula f = mngr.and(state1Formula, state2Formula);
  ...
  private AbstractEdge prepareEdge(AbstractEdge pEdge) {
    ...
    creationTimer.start();
    if (edge instanceof CFunctionReturnEdge) { /* 这里只考虑函数返回边和C表达式边 */
      CFunctionSummaryEdge summary = ((CFunctionReturnEdge) edge).getSummaryEdge();
    ...
    if (stmnt instanceof CAssignment) {
      ...
      //如果赋值表达式是一个涉及全局变量的表达式才可能返回非空的边，否则返回空边
      if (left.getSecond()) {
        ...
        if (havocAbstraction2) {
          ...
          newAssignement =
              new CFunctionCallAssignmentStatement(  /* 这里fExp充当righthand，因而该未解释函数没有参数） */
                  fExp.getFileLocation(),  /* 未解释函数的位置 */
                  (CLeftHandSide) left.getFirst(),  /* lefthand */
                  fExp);  /* righthand */
        } else {
        ...
        PathFormula pFormula = pmngr.makeEmptyPathFormula();   // above, modify the return types, int* -> int_p_, etc
                                                               // 将pdev = 5类型的赋值表达式变成pdev = __VERRIFIER_nondet_signed_int()类型等
        try {
          pFormula = pmngr.makeAnd(pFormula, fakeEdge); //got formula for input edge
  ...
  private BooleanFormula prepareFormula(BooleanFormula formula) { /* 返回投影中使用到的guard */
    if (emptyAbstraction) {
