...
public final class Plan_C_threadingTransferRelatoin extends SingleEdgeTransferRelation {
    ...
    @Override
    public Collection<Plan_C_threadingState> getAbstractSuccessorsForEdge(
            AbstractState pState, Precision precision, CFAEdge cfaEdge)
            throws CPATransferException, InterruptedException {
        Preconditions.checkNotNull(cfaEdge);

        Plan_C_threadingState state = (Plan_C_threadingState) pState;

        Plan_C_threadingState threadingState = exitThreads(state); // 已经结束的线程需要退出
        ...
        Collection<Plan_C_threadingState> results = getAbstractSuccessorsFromWrappedCPAs(
     ...
     */
    @Nullable
    private String getActiveThread(final CFAEdge cfaEdge, final Plan_C_threadingState threadingState) {
     ...
     */
    private Collection<Plan_C_threadingState> getAbstractSuccessorsForEdge0(
            final CFAEdge cfaEdge, final Plan_C_threadingState threadingState,
            final String activeThread, final Collection<Plan_C_threadingState> results)
    ...
    private Collection<Plan_C_threadingState> getAbstractSuccessorsFromWrappedCPAs(
    ...
    private Plan_C_threadingState exitThreads(Plan_C_threadingState tmp) {      // 已经退出的线程需要被移除
    ...
    private Collection<Plan_C_threadingState> startNewThread(
            final Plan_C_threadingState threadingState, final AStatement statement,
            final Collection<Plan_C_threadingState> results) throws UnrecognizedCodeException, InterruptedException {
        ...
        if (useAllPossibleClones) {
            ...
            final Collection<Plan_C_threadingState> newResults = new ArrayList<>();
            Set<Integer> usedNumbers = threadingState.getThreadNums();
            for (int i = Plan_C_threadingState.MIN_THREAD_NUM; i < maxNumberOfThreads; i++) {
    ...
    private Collection<Plan_C_threadingState> createThreadWithNumber(
            final Plan_C_threadingState threadingState,
            CIdExpression id,
            String functionName,
            int newThreadNum,
            final Collection<Plan_C_threadingState> results)
            throws UnrecognizedCodeException, InterruptedException {
        ...
        final Collection<Plan_C_threadingState> newResults = new ArrayList<>();
        for (Plan_C_threadingState ts : results) {
            Plan_C_threadingState newThreadingState = addNewThread(ts, threadId, newThreadNum, functionName);
     ...
     */
    @Nullable Plan_C_threadingState addNewThread(
            Plan_C_threadingState threadingState, String threadId, int newThreadNum, String functionName)
...
//        final String threadNameWithoutNum= threadId.replaceAll("\\d+", "");
    ...
    private Collection<Plan_C_threadingState> addLock(final Plan_C_threadingState threadingState, final String activeThread,
                                               String lockId, final Collection<Plan_C_threadingState> results) {
...
//  String lockId = ((CIdExpression) expr0).getName();
    ...
    private Collection<Plan_C_threadingState> removeLock(
    ...
    private Collection<Plan_C_threadingState> joinThread(Plan_C_threadingState threadingState,
                                                  AStatement statement, Collection<Plan_C_threadingState> results) throws UnrecognizedCodeException {
    ...
    private @Nullable Plan_C_threadingState handleLocalAccessLock(CFAEdge cfaEdge, final Plan_C_threadingState threadingState,
                                                           String activeThread) {
    ...
    public Collection<? extends AbstractState> strengthen(
            ...
            Precision precision)
            throws CPATransferException, InterruptedException {
        Optional<Plan_C_threadingState> results = Optional.of((Plan_C_threadingState) state);
    ...
    private @Nullable Plan_C_threadingState handleWitnessAutomaton(
            Plan_C_threadingState ts, AutomatonState automatonState) {
