...
public class ARGApplyOperator implements ApplyOperator {
  ...
  /**
   * 这里增加判断相容的函数，跳转到CompositeApplyOperator
   */
  public boolean compatible(AbstractState pTransitionA, AbstractState pTransitionB) {
    ARGState transitionA = (ARGState) pTransitionA;
    ARGState transitionB = (ARGState) pTransitionB;
    if(wrappedApply.compatible(transitionA.getWrappedState(), transitionB.getWrappedState())) {
      return true;
    }
    else
      return false;
