...
public class BAMTransferRelation extends AbstractBAMTransferRelation<CPAException> {
  ...
  protected Collection<? extends AbstractState> getWrappedTransferSuccessor(
    ...
    /**
     * 打印一下获取到的新状态，看是否包含lineNum=1446的
     */
    for(AbstractState a : result)
    {
      if(((LocationState)AbstractStates.extractStateByType(a, LocationState.class)).getLocationNode().getNumLeavingEdges() > 0 && ((LocationState)AbstractStates.extractStateByType(a, LocationState.class)).getLocationNode().getLeavingEdge(0).getFileLocation().getStartingLineNumber() == 1446)
      {
        System.out.println("");
      }
    }
