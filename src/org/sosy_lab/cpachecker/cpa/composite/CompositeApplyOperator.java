...
public class CompositeApplyOperator implements ApplyOperator {
  ...
  /**
   * 增加判断相容的函数，从ARGApplyOperator跳转过来
   */
  public boolean compatible(AbstractState pTransitionA, AbstractState pTransitionB) {
    CompositeState transitionA = (CompositeState) pTransitionA;
    CompositeState transitionB = (CompositeState) pTransitionB;

    assert (transitionA.getNumberOfStates() == transitionB.getNumberOfStates());

    Iterator<AbstractState> it1 = transitionA.getWrappedStates().iterator();
    Iterator<AbstractState> it2 = transitionB.getWrappedStates().iterator();

    //如果封装的状态中有一对不相容，则不相容
    for(ApplyOperator applyOp : applyOperators){
      AbstractState absState1 = it1.next();
      AbstractState absState2 = it2.next();
      if(!applyOp.compatible(absState1, absState2)) {
        return false;
      }
    }
    return true;
  ...
  public AbstractState project(AbstractState pParent, AbstractState pChild, AbstractEdge pEdge) {
    ...
    ImmutableList.Builder<AbstractState> appliedStates = ImmutableList.builder();   //承接最后的结果
    Iterator<AbstractState> iter1 = parent.getWrappedStates().iterator();   //parent和child都是复合的状态，需要对每一个子状态都进行相应的操作
