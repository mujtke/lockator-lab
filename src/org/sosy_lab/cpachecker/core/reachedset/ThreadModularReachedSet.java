...
public class ThreadModularReachedSet extends ForwardingReachedSet {
  ...
  public Collection<AbstractState> getStatesForApply(AbstractState pState) {    //获取用于apply操作的状态
    Preconditions.checkNotNull(pState);
    AbstractStateWithLocations loc =
        AbstractStates.extractStateByType(pState, AbstractStateWithLocations.class);
    if (loc instanceof AbstractStateWithEdge) {
      AbstractStateWithEdge locEdge = (AbstractStateWithEdge) loc;      //根据transition的组成，投影 = 抽象state + edge
      if (locEdge.isProjection()) {
        return ImmutableSet.copyOf(threadTransitions);                  //如果是投影就返回transitions集合
      } else if (locEdge.getAbstractEdge() instanceof WrapperCFAEdge) {
        return ImmutableSet.copyOf(projections);                        //如果不是投影则返回projections，投影集合
      } else {                                                          //如果pState是applied state则返回空集
        // applied state, skip
      }
    }
  ...
  /**
   * 增加一个返回threadTransitions的方法
   */
  public Collection<AbstractState> getTransitions() {
